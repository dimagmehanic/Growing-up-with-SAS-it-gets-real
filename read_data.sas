...
/*??????? ?????????*/
proc sort data =  VAERS_IDS dupout = kl nodupkey;
...
data mm;
  set  VAERS_IDS;
  by VAERS_ID   Disease TAKEN;
   length dd $200;retain dd;
  if first.VAERS_ID then dd = strip(Disease);
  if TAKEN = 'Multiple' and not first.VAERS_ID  then 
    dd = strip(Dd)||","||strip(Disease);
  else dd = Disease; 
  if last.VAERS_ID;  
run;
/*????????? ?????????????? ??????? ??????? ?? 2 ? ?????? ???????????(?? ????? ?
?????? ???? ??????????? ?? ?????? ????? ?????????????? ?????????? 1 ?? ?????????)*/
proc sql ; 
   select distinct dd into : combo separated by '@' from mm where TAKEN = 'One' and countw(dd,',') >=2;
   select count(distinct dd) into : n_combo separated by '@' from mm where TAKEN = 'One' and countw(dd,',') >=2;
quit;
/*????????? ?????? ??? ? ??? ????? ????????? ? ??????????? ????????
...
%macro keeps;
  data keeps ;
     ...
     %do i = 1 %to  &n_combo;
	  ...
	  if TAKEN = 'One' and dd = scan(www,&i,'@') then do; y = "Y";dd1 =dd;end; 
        ...
        if TAKEN = 'Multiple' then do;
		   ...
		   n = countw(tt,',') ;
           do i = 1 to n;
		      if find(dd,strip(scan(tt, i ,',' )),'i') then nn = nn+1;			  
		   end;
		   do k = 1 to countw(dd,',') ;
		      
		        if  not find(tt,strip(scan(dd, k , ',')),'i' ) then nn= nn+.0001; 		  
		      		      
		   end;

           if n  = nn   then do;  y = "Y";  dd1 = tt; end;
		end;
